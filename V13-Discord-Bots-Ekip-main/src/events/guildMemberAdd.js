            ...
            await data.findOne({ guild: member.guild.id }, async (err, res) => {
                {
                    ...
                    this.client.channels.cache.get(server.RegisterChat).send(`${this.client.emojis.cache.find(x => x.name === "gokw_star")} **${member.guild.name}** sunucumuza hoş geldin ${member} (\`${member.id}\`) .tag yazarak taglarımıza ulaşabilirsin, İyi Eğlenceler!
